...
trait QueriesRelationships
{
    ...
    public function has(
        ...
        if (
            ...
            $this->isAcrossConnections($relation)
        ) {
            return $this->addHybridHas(
                $relation,
                $operator,
                $count,
                $boolean,
    ...
    {
        return $relation->getParent()->getConnectionName() !==
    ...
    public function addHybridHas(
        ...
        $relatedIds = $this->getConstrainedRelatedIds(
            $relations,
            $operator,
            $count
        );
        ...
        return $this->whereIn(
            $this->getRelatedConstraintKey($relation),
            $relatedIds,
            $boolean,
            $not
        );
    ...
    {
        $relationCount = array_count_values(
            array_map(
                function ($id) {
                ...
                is_array($relations)
                    ? $relations
                    : $relations->flatten()->toArray()
            )
        );
        // Remove unwanted related objects based on the operator and count.
        $relationCount = array_filter($relationCount, function ($counted) use (
            $count,
            $operator
        ) {
