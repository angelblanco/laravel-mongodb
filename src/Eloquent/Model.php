...
use AngelBlanco\Mongodb\Casts\ObjectIdCast;
use Illuminate\Contracts\Queue\QueueableEntity;
use AngelBlanco\Mongodb\Casts\ObjectIdArrayCast;
use Illuminate\Contracts\Queue\QueueableCollection;
use Illuminate\Database\Eloquent\Model as BaseModel;
use Illuminate\Database\Eloquent\Relations\Relation;
use AngelBlanco\Mongodb\Query\Builder as QueryBuilder;

abstract class Model extends BaseModel
{
    use HybridRelations;

    const CAST_ID = ObjectIdCast::class;
    const CAST_ID_ARRAY = ObjectIdArrayCast::class;
        ...
        foreach ($this->getDates() as $key) {
            if (Str::contains($key, '.') && Arr::has($attributes, $key)) {
                Arr::set(
                    $attributes,
                    $key,
                    (string) $this->asDateTime(Arr::get($attributes, $key))
                );
            }
        }
    ...
    public function originalIsEquivalent($key)
    {
        ...
        if ($this->isDateAttribute($key)) {
            $attribute =
                $attribute instanceof UTCDateTime
                    ? $this->asDateTime($attribute)
                    : $attribute;
            $original =
                $original instanceof UTCDateTime
                    ? $this->asDateTime($original)
                    : $original;
     ...
     * @param bool   $unique
     */
    protected function pushAttributeValues(
        $column,
        array $values,
        $unique = false
    ) {
