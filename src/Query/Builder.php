...
namespace AngelBlanco\Mongodb\Query;
...
class Builder extends BaseBuilder
{
    ...
    {
        return $this->where('_id', '=', $this->convertKey($id))->first(
            $columns
        );
    }
        ...
        if ($this->groups || $this->aggregate) {
            ...
            if ($this->aggregate) {
                ...
                foreach ($this->aggregate['columns'] as $column) {
                    ...
                    if (2 == count($splitColumns = explode('.*.', $column))) {
                    ...
                    $aggregations = blank($this->aggregate['columns'])
                        ? []
                        : $this->aggregate['columns'];

                    if (in_array('*', $aggregations) && 'count' == $function) {
    ...
    {
        $results = $this->get(null === $key ? [$column] : [$column, $key]);
    ...
    {
        if (is_string($id) && 24 === strlen($id) && ctype_xdigit($id)) {
        ...
        foreach ($wheres as $i => &$where) {
            ...
            if (isset($where['value'])) {
                if (is_array($where['value'])) {
                    array_walk_recursive($where['value'], function (
                        &$item,
                        $key
                    ) {
            ...
            if (0 == $i && count($wheres) > 1 && 'and' == $where['boolean']) {
    ...
    protected function compileWhereBasic(array $where)
    {
        ...
        elseif (
            in_array($operator, ['regexp', 'not regexp', 'regex', 'not regex'])
        ) {
