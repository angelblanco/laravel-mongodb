...
namespace AngelBlanco\Mongodb\Tests\Query;
...
class QueryBuilderTest extends TestCase
{
    ...
    public function testNoDocument()
    {
        ...
        $item = DB::collection('items')
            ->where('_id', '51c33d8981fec6813e00000a')
            ->first();
        $this->assertNull($item);
    }
    ...
    public function testUpdate()
    {
        ...
        $john = DB::collection('users')
            ->where('name', 'John Doe')
            ->first();
        $jane = DB::collection('users')
            ->where('name', 'Jane Doe')
            ->first();
    ...
    public function testPull()
    {
        ...
        DB::collection('users')
            ->where('_id', $id)
            ->pull(['tags' => 'tag2', 'messages' => $message2]);
        $user = DB::collection('users')->find($id);
        $this->assertCount(2, $user['tags']);
    ...
    public function testList()
    {
        ...
        $this->assertEquals(
            ['knife' => 'sharp', 'fork' => 'sharp', 'spoon' => 'round'],
            $list
        );
    ...
    public function testSubdocumentAggregate()
    {
        ...
        $this->assertEquals(
            16.25,
            DB::collection('items')->avg('amount.hidden')
        );
    }
    ...
    public function testSubdocumentArrayAggregate()
    {
        ...
        $this->assertEquals(
            72,
            DB::collection('items')->sum('amount.*.hidden')
        );
        $this->assertEquals(
            6,
            DB::collection('items')->count('amount.*.hidden')
        );
        $this->assertEquals(1, DB::collection('items')->min('amount.*.hidden'));
        $this->assertEquals(
            35,
            DB::collection('items')->max('amount.*.hidden')
        );
        $this->assertEquals(
            12,
            DB::collection('items')->avg('amount.*.hidden')
    ...
    public function testUpsert()
    {
        DB::collection('items')
            ->where('name', 'knife')
            ->update(['amount' => 1], ['upsert' => true]);

        $this->assertEquals(1, DB::collection('items')->count());

        Item::where('name', 'spoon')->update(
            ['amount' => 1],
            ['upsert' => true]
        );
    ...
    {
        $id1 = DB::collection('users')->insertGetId([
            'name' => 'John Doe',
            'note1' => 'ABC',
            'note2' => 'DEF',
        ]);
        $id2 = DB::collection('users')->insertGetId([
            'name' => 'Jane Doe',
            'note1' => 'ABC',
            'note2' => 'DEF',
        ]);
    ...
    public function testUpdateSubdocument()
    {
        ...
        DB::collection('users')
            ->where('_id', $id)
            ->update(['address.country' => 'England']);
    ...
    public function testDates()
    {
        DB::collection('users')->insert([
            [
                'name' => 'John Doe',
                'birthday' => new UTCDateTime(
                    Date::parse('1980-01-01 00:00:00')->format('Uv')
                ),
            ],
            [
                'name' => 'Robert Roe',
                'birthday' => new UTCDateTime(
                    Date::parse('1982-01-01 00:00:00')->format('Uv')
                ),
            ],
            [
                'name' => 'Mark Moe',
                'birthday' => new UTCDateTime(
                    Date::parse('1983-01-01 00:00:00.1')->format('Uv')
                ),
            ],
            [
                'name' => 'Frank White',
                'birthday' => new UTCDateTime(
                    Date::parse('1960-01-01 12:12:12.1')->format('Uv')
                ),
            ],
        ]);
        ...
        $user = DB::collection('users')
            ->where('birthday', '=', new DateTime('1980-01-01 00:00:00'))
            ->first();
        $this->assertEquals('John Doe', $user['name']);
        ...
        $users = DB::collection('users')
            ->whereBetween('birthday', [$start, $stop])
            ->get();
        $this->assertCount(2, $users);
    }
    ...
    public function testOperators()
    {
        ...
        $results = DB::collection('items')
            ->where('tags', 'all', ['sharp', 'pointy'])
            ->get();
        $this->assertCount(2, $results);

        $results = DB::collection('items')
            ->where('tags', 'all', ['sharp', 'round'])
            ->get();
        $this->assertCount(1, $results);
    ...
    public function testValue()
    {
        ...
        $this->assertEquals(
            'Moby-Dick',
            DB::collection('books')->value('title')
        );
        $this->assertEquals(
            ['first_name' => 'Herman', 'last_name' => 'Melville'],
            DB::collection('books')->value('author')
        );
        $this->assertEquals(
            'Herman',
            DB::collection('books')->value('author.first_name')
        );
        $this->assertEquals(
            'Melville',
            DB::collection('books')->value('author.last_name')
        );
    }
